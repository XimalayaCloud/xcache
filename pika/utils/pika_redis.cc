...
int rdbLoadObject(int rdbtype, rio *rdb, restore_value *dbvalue) {
    ...
    if (rdbtype == REDIS_RDB_TYPE_STRING) {
        /* Read string value */
        dbvalue->type = blackwidow::kStrings;
        std::string value;
        ...
        /* Read list value */
        size_t len;
        dbvalue->type = blackwidow::kLists;
        ...
        /* Read list/set value */
        size_t len;
        dbvalue->type = blackwidow::kZSets;
        if ((len = rdbLoadLen(rdb, NULL)) == REDIS_RDB_LENERR){
            LOG(ERROR) << "REDIS_RDB_TYPE_SET : get len error";
            return REDIS_ERR;
        }
        ...
        size_t len;
        dbvalue->type = blackwidow::kHashes;
        if ((len = rdbLoadLen(rdb, NULL)) == REDIS_RDB_LENERR){
            LOG(ERROR) << "REDIS_RDB_TYPE_SET : get len error";
            return REDIS_ERR;
        }
               ...
               rdbtype == REDIS_RDB_TYPE_HASH_ZIPLIST)
    {
        ...
        dbvalue->type = blackwidow::kNONE_DB;
    }
