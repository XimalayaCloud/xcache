...
void
PikaZsetAutoDelThread::RequestCronTask() {
    slash::MutexLock l(&mutex_);
...
void
PikaZsetAutoDelThread::RequestManualTask(int64_t cursor, double speed_factor) {
...
void
PikaZsetAutoDelThread::StopManualTask() {
...
void PikaZsetAutoDelThread::GetZsetInfo(ZsetInfo &info) {
    ...
    info.current_task_spend_time = slash::GetCurrentTime() - start_check_all_zset_time_;
    info.current_cursor = current_cursor_;
}

void
PikaZsetAutoDelThread::CompactZsetDB() {
    ...
    rocksdb::Status s = g_pika_server->db()->Compact(blackwidow::kZSets);
    if (s.ok()) {
...
void PikaZsetAutoDelThread::TrimAllZsetKeysFinished() {
    ...
    last_finish_check_all_zset_time_ = slash::GetCurrentTime();
...
void
PikaZsetAutoDelThread::WriteZsetAutoDelBinlog(const std::string &key, int start, int end) {
    std::string raw_args;
        ...
        if (s.ok() && zset_size > zset_auto_del_threshold) {
...

            {
                ...
                if (s.ok()) {
                    ...
                    // if in cache model, delete from cache
                    if (PIKA_CACHE_NONE != g_pika_conf->cache_model()
                        && PIKA_CACHE_STATUS_OK == g_pika_server->Cache()->CacheStatus()) {
                        std::string min = std::to_string(start);
                        std::string max = std::to_string(end);
                        g_pika_server->Cache()->ZRemrangebyrank(key, min, max);
                    }
...
void
PikaZsetAutoDelThread::DoZsetCronTask(double speed_factor) {
    if (0 == current_cursor_) {
        start_check_all_zset_time_ = slash::GetCurrentTime();
    }
...
void
PikaZsetAutoDelThread::DoZsetManualTask(int64_t cursor, double speed_factor) {
    start_check_all_zset_time_ = slash::GetCurrentTime();
...
void
PikaZsetAutoDelThread::DoZsetAutoDelTask(ZsetTaskItem &task_item) {
...
void*
PikaZsetAutoDelThread::ThreadMain() {
    while (!should_exit_) {
