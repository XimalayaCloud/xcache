...
Status VersionSet::Recover() {
  ...
  manifest_name_ = "/titandb/" + manifest;
...
Status VersionSet::OpenManifest(uint64_t file_number) {
  ...
  manifest_name_ = DescriptorFileName("/titandb", file_number);
...
void VersionSet::GetTitanLiveFiles(std::vector<std::string>* ret) {
  ret->clear();
  ...
  for (const auto& bs : v->column_families_) {
    for (const auto& f : bs.second->files_) {
      auto file_name = BlobFileName("/titandb", f.second->file_number());
      ret->push_back(file_name);
    }
  }

  ret->push_back(CurrentFileName("/titandb"));
