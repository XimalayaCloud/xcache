...
Status TitanDBImpl::GetLiveFiles(std::vector<std::string>& vec,
                                 uint64_t* mfs,
                                 bool flush_memtable) {
  // get rocksdb files
  auto s = db_impl_->GetLiveFiles(vec, mfs, flush_memtable);

  // Get titan files
  std::vector<std::string> titan_files;
  vset_->GetTitanLiveFiles(&titan_files);
  
  vec.insert(vec.end(), titan_files.begin(), titan_files.end());
