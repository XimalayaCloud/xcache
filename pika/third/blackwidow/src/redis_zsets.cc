...
Status RedisZSets::ResetOption(const std::string& key, const std::string& value) {
  Status s = GetDB()->SetOptions(handles_[0], {{key,value}});
  if (!s.ok()) {
    return s;
  }
  s = GetDB()->SetOptions(handles_[1], {{key,value}});
  if (!s.ok()) {
    return s;
  }
...
Status RedisZSets::ScanKeyNum(uint64_t* num) {
  ...
  for (iter->SeekToFirst();
    ...
    if (parsed_zsets_meta_value.count() != 0
      && !parsed_zsets_meta_value.IsStale()) {
      count++;
    }
  }
  *num = count;
...
Status RedisZSets::ZAdd(const Slice& key,
  ...
  *ret = 0;
  ...
  if (s.ok()) {
    ...
    if (parsed_zsets_meta_value.count() == 0 || parsed_zsets_meta_value.IsStale()) {
      is_stale = true;
...
Status RedisZSets::ZRangeWithTTL(const Slice& key,
                                 int32_t start,
                                 int32_t stop,
                                 std::vector<ScoreMember>* score_members,
                                 int64_t* ttl) {
