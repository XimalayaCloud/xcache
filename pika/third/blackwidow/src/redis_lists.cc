...
Status RedisLists::ResetOption(const std::string& key, const std::string& value) {
...
Status RedisLists::ScanKeyNum(uint64_t* num) {
  ...
  for (iter->SeekToFirst();
    ...
    if (parsed_lists_meta_value.count() != 0
      && !parsed_lists_meta_value.IsStale()) {
      count++;
    }
  }
  *num = count;
...
Status RedisLists::LPush(const Slice& key,
  ...
  *ret = 0;
  ...
  if (s.ok()) {
    ParsedListsMetaValue parsed_lists_meta_value(&meta_value);
    if (parsed_lists_meta_value.count() == 0 || parsed_lists_meta_value.IsStale()) {
...
Status RedisLists::LRangeWithTTL(const Slice& key, int64_t start, int64_t stop,
