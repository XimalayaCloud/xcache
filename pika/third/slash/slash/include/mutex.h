...
class ScopeRecCondVar {
 public:
  virtual ~ScopeRecCondVar() {}
  ...
  virtual Status Wait(std::shared_ptr<ScopeRecMutex> mutex) = 0;
  ...
  //  fail the operation.
  // May return OK spuriously even if not notified.
  virtual Status WaitFor(std::shared_ptr<ScopeRecMutex> mutex,
...
class MutexFactory {
 public:
  ...
  virtual std::shared_ptr<ScopeRecMutex> AllocateMutex() = 0;

  // Create a CondVar object.
  virtual std::shared_ptr<ScopeRecCondVar> AllocateCondVar() = 0;

  virtual ~MutexFactory() {}
};

}  // namespace slash
#endif  // SLASH_MUTEX_H_
