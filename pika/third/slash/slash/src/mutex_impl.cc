...
class MutexImpl : public ScopeRecMutex {
 public:
  MutexImpl() {}
  ~MutexImpl() {}
...
class CondVarImpl : public ScopeRecCondVar {
 public:
  CondVarImpl() {}
  ~CondVarImpl() {}

  Status Wait(std::shared_ptr<ScopeRecMutex> mutex) override;

  Status WaitFor(std::shared_ptr<ScopeRecMutex> mutex,
...
std::shared_ptr<ScopeRecMutex>
MutexFactoryImpl::AllocateMutex() {
  return std::shared_ptr<ScopeRecMutex>(new MutexImpl());
}

std::shared_ptr<ScopeRecCondVar>
MutexFactoryImpl::AllocateCondVar() {
  return std::shared_ptr<ScopeRecCondVar>(new CondVarImpl());
}
...
Status CondVarImpl::Wait(
    std::shared_ptr<ScopeRecMutex> mutex) {
...
Status CondVarImpl::WaitFor(
    std::shared_ptr<ScopeRecMutex> mutex, int64_t timeout_time) {
  Status s;
